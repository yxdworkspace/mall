...
<script>
  ...
  import store from "@/store";
    ...
    computed: {
		  totalPrice() {
        const cartList = store.state.cartShop;
      ...
      isSelectAll: function () {
        return store.state.cartShop.find(item => item.checked === false) === undefined;
      },
      cartCount(){
    ...
    methods: {
      checkBtnClick: function () {
        // 1.判断是否有未选中的按钮
        let isSelectAll = store.state.cartShop.find(item => !item.checked);

        // 2.有未选中的内容, 则全部选中
        if (isSelectAll) {
          store.state.cartShop.forEach(item => {
            item.checked = true;
          });
        } else {
          store.state.cartShop.forEach(item => {
